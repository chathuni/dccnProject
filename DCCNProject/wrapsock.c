...
again:
	if ( (n = accept(fd, sa, salenptr)) < 0) {//If accept() returns following errors:redo the process
#ifdef	EPROTO 
...
void
Bind(int fd, const struct sockaddr *sa, socklen_t salen)//If bine returns -value:print bind error.
{
...
void
Connect(int fd, const struct sockaddr *sa, socklen_t salen)//If connect returns -value:print err msg
{
...
{
	if (getpeername(fd, sa, salenptr) < 0)	//returns -value:print err msg
...
{
	if (getsockname(fd, sa, salenptr) < 0)//returns -value:print err msg
...
{
	if (getsockopt(fd, level, optname, optval, optlenptr) < 0)//returns -value:print err msg
